...
delimiter //
	create procedure scheldule_appointment(IN app_id INT,IN p_id INT, IN d_id INT,IN app_date DATETIME,
					app_status ENUM('Schelduled','Completed','Cancelled'))
    begin
   		if exists (SELECT * FROM patient WHERE patient_id = p_id) THEN
			insert into appoinment (appoinment_id,patient_id, doctor_id, appoinment_date,status)  -- appoinment_id,
			values (app_id,p_id,d_id,app_date,app_status);  
...
delimiter ;
call scheldule_appointment(4,5,4,now(),'Schelduled');    
